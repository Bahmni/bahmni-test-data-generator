		...
		if (patientGender.equals("F") && patientAge > 14 && patientAge < 40) {
			...
			if (chancePercent(3f)) {
				encounters.add(new Encounter(registrationDate)
						.setCondition(Coding.of(selectRandomDescendantOf("77386006"), Coding.SNOMED_URI, "Pregnancy (finding)")));// 77386006 |Pregnancy (finding)|
				...
				if (chancePercent(78f)) {
					...
					if (chancePercent(23f)) {
						...
						encounters.add(new Encounter(registrationDate)
								.setDiagnosis(Coding.of(selectRandomDescendantOf("87522002"), Coding.SNOMED_URI, "Iron deficiency anemia (disorder)")));// 87522002 |Iron deficiency anemia (disorder)|

						if (chancePercent(7f)) {
							// Premature delivery
							encounters.add(new Encounter(registrationDate)
									.setDiagnosis(Coding.of(selectRandomDescendantOf("282020008"), Coding.SNOMED_URI, "Premature delivery (finding)")));// 282020008 |Premature delivery (finding)|
						}
					} else {
					...
					if (chancePercent(5.5f)) {
						// Pre-eclampsia
						encounters.add(new Encounter(registrationDate)
								.setDiagnosis(Coding.of(selectRandomDescendantOf("398254007"), Coding.SNOMED_URI, "Pre-eclampsia (disorder)")));// 398254007 |Pre-eclampsia (disorder)|
					}
